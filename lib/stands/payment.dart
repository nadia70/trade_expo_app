...
import 'package:expo_app/userScreens/login.dart';
import 'package:expo_app/models/alert_dialog.dart';
import 'package:expo_app/models/channel_methods.dart';
import 'package:expo_app/tools/app_data.dart';
import 'package:expo_app/userScreens/address.dart';
import 'package:expo_app/main.dart';
import 'package:expo_app/models/base.dart';
...
class _SupportOrdersState extends State<OrderSummary> {
      ...
      action: new SnackBarAction(
          ...
          onPressed: () {
            Navigator.of(context).push(new CupertinoPageRoute(
                builder: (BuildContext context) => new ExpoLogin()));
          }),
    ))
        : scaffoldKey.currentState.showSnackBar(new SnackBar(
      content: new Text(value),
  ...
  showAlertDialog() {
    var alertDialog = new CustomAlertDialog(
      ...
      onProceed: () {
        ...
        Navigator.of(context).push(new CupertinoPageRoute(
            builder: (BuildContext context) => new ExpoAddress(
              fromPayment: true,
            )));
      },
    );
  ...
  Future openPaystackPortal() async {
    ...
    String result;
    try {
      result = await ExpoChannels.connectToPaystack({
        "NAME": "Your Name",
        "EMAIL": "you@email.com",
        "AMOUNT": 100,
        "CURRENCY": "NGN",
